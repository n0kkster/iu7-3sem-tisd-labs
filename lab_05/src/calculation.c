...
// Псевдо таймер работы программы
...
static double calc_error_total_time(void)
{
    double error = (simulation_info.total_time / (double)(simulation_params.total_tasks_count * (MAX(simulation_params.max_task_generation_interval, simulation_params.max_task_processing_time * simulation_params.task_repeat) / 2.0)) - 1) * 100;
    return error > 0 ? error : -error;
}

// Функция для подсчета погрешности по количеству выполненных задач
static double calc_error_tasks_count(void)
{
...
static double calc_error_sum_times(void)
{
...
void init_calculation(void *queue_ptr, error_codes_e (*push)(void *, task_t), error_codes_e (*pop)(void *, task_t *), bool (*is_empty)(void *), size_t (*get_queue_len)(void *))
{
    // Обнуление таймера
    pseudo_clock = 0;
    ...
    pseudo_clock = generator.interval;
}
...
static error_codes_e update_generator(generator_t *generator)
{
    ...
    {
        // ... и оно уже наступило
        if (fabs(pseudo_clock - generator->prev_ready_moment - generator->interval) <= 1e-6)
        {
...
static error_codes_e update_processor(processor_t *processor)
{
    ...
    if (processor->state == BUSY)
    {
        ...
        if (fabs(pseudo_clock - processor->started_moment - processor->current_task.execution_time) <= 1e-6)
        {
            ...
            else
            {
                ...
                if (simulation_info.processed_tasks % simulation_params.print_queue_info_frequency == 0)
                {
                    ...
                    printf("Средняя длина очереди: %.3lf\n", simulation_info.average_queue_len / update_count);
...
error_codes_e update_calculation(void)
{
    error_codes_e rc;
    ...
    // Обновления количества циклов запуска
    update_count++;

    // Если процессор простивается и очередь пусть
    if (processor.state == IDLE && queue.get_queue_len(queue.queue) == 0)
    {
        // То увеличить счетчик общего простоя процессора
        simulation_info.total_processor_idle_time += generator.prev_ready_moment + generator.interval - pseudo_clock;
        
        // И перейти к моменту, когда будет готова следующая задача
        pseudo_clock = generator.prev_ready_moment + generator.interval;
    }
    
    // Если же процессор занят
    else if (processor.state == BUSY)
    {
        // Если текущая задача выполнится раньше готовности генератора
        if (processor.started_moment + processor.current_task.execution_time <= generator.prev_ready_moment + generator.interval)
            // То перейти к моменту заврешения текущей задачи
            pseudo_clock = processor.current_task.execution_time + processor.started_moment;
        else
            // Иначе перейти к моменту готовности генератора
            pseudo_clock = generator.prev_ready_moment + generator.interval;
    }
...
error_codes_e run_calculation(void *queue_ptr, error_codes_e (*push)(void *, task_t), error_codes_e (*pop)(void *, task_t *), bool (*is_empty)(void *), size_t (*get_queue_len)(void *), simulation_params_t *params)
{
    ...
    init_calculation(queue_ptr, push, pop, is_empty, get_queue_len);
    ...
    while (simulation_info.processed_tasks < simulation_params.total_tasks_count)
        if ((rc = update_calculation()) != ERROR_SUCCESS)
            return rc;
